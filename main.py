...
def refresh_members():
    ...
    for member in members:
        ...
        members_list.insert(
            tk.END,
            f"ID:{member[0]} | Name:{member[1]} | Email:{member[2]}"
        )
...
def refresh_borrowings():
    ...
    for loan in loans:
        ...
        status = "Returned" if loan[4] else "Borrowed"

        loans_list.insert(
            tk.END,
            f"ID:{loan[0]} | Book:{loan[1]} | Member:{loan[2]} | Date:{loan[3]} | {status}"
        )
...
def delete_borrowing():
    ...
    data = loans_list.get(selected[0])

    loan_id = data.split("|")[0].replace("ID:", "").strip()
